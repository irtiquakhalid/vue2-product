  ...
  <div class="add-task">
    ...
    <v-row>
      <v-col cols="12" md="6">
        ...
        <v-combobox
          ...
          hide-details
          :disabled="taskDeadlineValue.type == 'edit'"
      ...
      <v-col cols="12" md="6">
        <small class="cyan-text font-18">Role</small>
        <v-select
          solo
          flat
          class="mt-2 radius-7 custom-select"
          hide-details
          background-color="var(--secondary-color)"
    ...
    <v-row>
      <v-col cols="12" md="6">
        ...
        <small class="cyan-text font-18">Early Notice</small>
        <div class="">
          <v-menu
            ref="menu1"
            v-model="menu1"
            :close-on-content-click="false"
            :return-value.sync="form.taskStartDate"
          ...
          >
            ...
            >
              ...
              <v-btn
                text
                color="primary"
                @click="$refs.menu1.save(form.taskStartDate)"
              >
                OK
              </v-btn>
            </v-date-picker>
      ...
      <v-col cols="12" md="6">
        <!-- Date Picker To -->
        <div>
          <small class="cyan-text font-18">Deadline</small>
          <v-menu
            ref="menu"
            v-model="menu"
            :close-on-content-click="false"
            :return-value.sync="form.taskDeadLine"
            transition="scale-transition"
            offset-y
            left
            min-width="auto"
          ...
          >
            ...
            >
              ...
              <v-btn
                text
                color="primary"
                @click="$refs.menu.save(form.taskDeadLine)"
              >
                OK
              </v-btn>
            </v-date-picker>
      ...
      <v-col cols="12" class="mt-4">
        ...
        <div class="attach-document">
          <svg
            ...
            v-if="!documentName && !form.filePath"
          >
            <rect
              x="0.628571"
              y="0.628571"
          ...
          <h6 v-if="documentName || form.filePath">
            {{ documentName || form.filePath }}
          </h6>
          <input
            type="file"
            v-if="taskDeadlineValue.type !== 'edit'"
...
export default {
  ...
  computed: {
    ...mapGetters(["getUserId", "getRoles", "getTasks"]),
  },
  mounted() {
    this.mapData();
    this.fetchdata();
  ...
  watch: {
    ...
    "form.taskDeadLine": {
      immediate: true,
      handler() {
        this.maxStartDate = this.form.taskDeadLine;
      },
    },
    "form.taskStartDate": {
      immediate: true,
      handler() {
        this.minEndDate = this.form.taskStartDate;
      },
    },
  },
  methods: {
    getAllTasksForSuggestions() {
      this.loader = true;
      this.$store
        .dispatch("getTasks", { isPagination: false })
        .then((response) => {
    ...
    saveDeadline() {
      if (this.taskDeadlineValue.type == "add") {
        if (this.$v.$invalid) {
          this.$v.$touch();
        } else {
          this.loader = true;
          ...
          this.$store.dispatch("saveCustomTask", payload).then((response) => {
            if (response.data.succeeded === true) {
              ...
              let taskData = {
                ...response.data.data,
                taskStartDate: this.form.taskStartDate,
                taskDeadLine: this.form.taskDeadLine,
              };
              this.$emit("addTaskSoft", taskData);
            } else {
              this.loader = false;
              this.$swal.fire({
                position: "top-end",
        ...
        this.$emit("updateDatesOnly", this.form);
        this.close();
        this.resetForm();
      }
    },
    ...
    mapData() {
      if (this.taskDeadlineValue.type == "edit") {
        this.loader = true;
        this.form = { ...this.taskDeadlineValue.data };
        setTimeout(() => {
          this.form.taskName = this.taskDeadlineValue?.data?.taskName;
        }, 800);
        this.loader = false;
      } else {
        this.resetForm();
