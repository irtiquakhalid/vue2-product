<template>
  <div class="add-task">
    ...
    <div class="d-flex justify-space-between mb-4">
      <h3 class="dynamic-color">Edit Schedule Checklist Task</h3>
      <v-icon
        @click="close()"
    ...
    <v-row no-gutters>
      ...
      >
        ...
        />
          ...
          <h6 v-if="form.filePath && !documentName">
...
export default {
  ...
  computed: {
    ...mapGetters(["getUserId", "getTasks", "getRoles"]),
  },
  mounted() {
    this.fetchdata();
  ...
  methods: {
    getAllTasksForSuggestions(){
      ...
      this.$store.dispatch('getTasks',{isPagination: false}).then((response) => {
         this.taskSuggestionsData = response.data?.data
         this.loader = false;
      })
    },
    fetchdata() {
      this.loader = true;
      this.$store.dispatch("getTasks", { id: this.editId }).then(() => {
        this.loader = false;
        this.form = this.getTasks;
      });
